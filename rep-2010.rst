...
Type descriptions will be sent as a response to any query of a dedicated ROS service server (``~/_get_type_description``) that will be started on initialization of each node on either end of a connection (e.g. publishers and/or subscriptions).
...
The service server will reply with the type description and any necessary metadata needed to do runtime type introspection.
