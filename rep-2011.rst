...
Specifically, this feature should allow access to the data and fields in a serialized message buffer when given:

- the raw ``uint8[]`` message buffer
- the message's type description, from inter-process type description distribution
- the version and name of the serialization library used to serialize the message
...
- access to the field by name or index as a type
...
Given that the scope of inputs and expected outputs is so limited, this feature should be implemented as a separate package (e.g., ``rcl_serialization``) that can be called independently by any downstream packages that might need runtime type introspection (e.g. ROS, rosbag transport, etc.)

Furthermore, it is important to note that this feature is distinguished from ROS 2's "dynamic" type support (``rosidl_typesupport_introspection_c``) in that "dynamic" type support generates type support code at compile time that then is not expected to change, whereas this feature aims to support any arbitrary message type descriptions at runtime.
...
- the producers (publishers) and consumers (subscribers) of the message are using compatible RMW implementations
- the consumer has access to a compatible version of the serialization library described in the message type description
...
As runtime type introspection is expected to work across any arbitrary serialization library, functionality has to be built in to allow for the loading of the necessary serialization libraries required by the message type description.
Serialization library support in this case will be provided by writing plugins that wrap the serialization libraries that can then plug in dynamically to the runtime type introspection feature using a unified plugin interface. Then, runtime type introspection will:
...
.. code::
    ...
    introspection_handle->deserialize(&plugin, message_buffer, &FooDescription, foo_msg);
